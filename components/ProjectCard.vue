<template>
  <div class="w-[400px] h-[650px] rounded-[16px] shadow-custom mx-[50px] mb-[70px] bg-white text-black">
    <img
      class="w-[400px] h-[210px] rounded-t-[16px] mb-[20px]"
      :src="data[0]"
      alt="project_image"
    />
    <h2 class="my-[20px] text-center text-[24px] font-semibold">
      {{ data[1] }}
    </h2>
    <div class="w-[350px] text-justify mx-auto mb-[50px]">
      <p>{{ data[2] }}</p>
    </div>
    <div class="flex flex-wrap justify-center items-center gap-[50px]">
      <a
        v-if="repo"
        target="_blank"
        rel="noopener noreferrer"
        :href="repo_link"
        class="transition-all duration-[0.2s] ease-out hover:scale-[1.1]"
      >
        <img
          class="w-[50px]"
          src="https://res.cloudinary.com/deotitxt8/image/upload/v1733417591/portfolio/ihtebuqsqrrgvd6fjiev.png"
          alt="repo_icon"
        />
      </a>

      <a
        v-if="doc"
        target="_blank"
        rel="noopener noreferrer"
        :href="doc_link"
        class="transition-all duration-[0.2s] ease-out hover:scale-[1.1]"
      >
        <img
          class="w-[50px]"
          src="https://res.cloudinary.com/deotitxt8/image/upload/v1733418875/portfolio/gclghq8ol3x1rmfecgdw.png"
          alt="doc_icon"
        />
      </a>

      <a
        v-if="web"
        target="_blank"
        rel="noopener noreferrer"
        :href="web_link"
        class="transition-all duration-[0.2s] ease-out hover:scale-[1.1]"
      >
        <img
          class="w-[50px]"
          src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/Globe_icon.svg/768px-Globe_icon.svg.png"
          alt="web_icon"
        />
      </a>

      <a
        v-if="video"
        target="_blank"
        rel="noopener noreferrer"
        :href="video_link"
        class="transition-all duration-[0.2s] ease-out hover:scale-[1.1]"
      >
        <img
          class="w-[50px]"
          src="https://res.cloudinary.com/deotitxt8/image/upload/v1741366417/portfolio/boton-de-play_y7yu16.png"
          alt="web_icon"
        />
      </a>
    </div>
  </div>
</template>

<script lang="ts" setup>
interface ProjectCardProps {
  data: string[];
  repo?: boolean;
  repo_link?: string;
  doc?: boolean;
  doc_link?: string;
  web?: boolean;
  web_link?: string;
  video?: boolean;
  video_link?: string;
}

defineProps<ProjectCardProps>();
</script>

<style scoped>
.shadow-custom {
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
}
</style>
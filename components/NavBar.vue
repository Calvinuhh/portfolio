<template>
  <nav
    class="relative mx-auto mt-10 mb-[100px]"
  ...
  >
    ...
    <div
      ref="navContainer"
      class="hidden md:flex items-center justify-center h-[90px] relative"
    >
      <button
      ...
      >
        <span
          class="text-[17px] font-medium tracking-wide whitespace-nowrap transition-colors duration-200"
          :class="{
            'text-[#4dcfe3]': activeSection === item.id,
            'group-hover:text-[#a5c190]': activeSection !== item.id,
          }"
        >
          {{ item.label }}
        </span>
    ...
    <div
      v-if="menuOpen"
      class="flex flex-col items-center mt-5 space-y-4 md:hidden"
    >
      <button
        v-for="item in navItems"
        :key="item.id"
